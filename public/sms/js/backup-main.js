...
if (document.getElementById("bt_form_votar")){
	var err_msg=document.getElementById("err_msg");
	var bt_form_votar= document.getElementById("bt_form_votar");
	bt_form_votar=bt_form_votar.getElementsByTagName("span")[0];
	var cerrar_err = err_msg.getElementsByClassName("cerrar")[0];

bt_form_votar.onclick=function(){
    var get_input = document.getElementsByClassName("quantity")[0].getElementsByTagName("input")[0];
    if (get_input.value > 0){
        var get_form = document.getElementById("bt_votar");
            get_form =get_form.getElementsByTagName("form")[0];
            $('<div id="pusiste_fichas" class="resaltado_gris"><span class="text_bold subrayado">Ya pusiste fichas</span><span class="icon-o"></span></div>').insertAfter('#bt_votar form');
            var get_msg_ok = document.getElementById("pusiste_fichas");
            get_form.style.display="none";
            $("#pusiste_fichas").fadeIn(1000).css("display","inline-block");
    }else{
        open_modal(err_msg);
    };
};
...
btn_ver_quien_fichas.onclick= function(){
    ...
    if( $(window).width() >= 590 && $(window).width() < 992 ){
            $("html, body").animate({ scrollTop: "831" }); 
...
for (var z = 0; z < grilla.length; z++){
var get_logo = document.getElementsByClassName("logo_particantes")[0];
var get_logo_est_clon = get_logo.cloneNode(true); 
var backup_estructura = get_logo.cloneNode(true); 
if (z==0){
get_logo.getElementsByTagName("img")[0].src = grilla[z].link;
get_logo.getElementsByTagName("img")[0].alt = "texto imagen";
get_logo.getElementsByTagName("h2")[0].innerHTML = grilla[z].titulo;
get_logo.getElementsByTagName("h3")[0].innerHTML = grilla[z].nombre;
get_logo.getElementsByTagName("img")[1].src = grilla[z].avatar;
get_logo.getElementsByTagName("img")[1].alt = grilla[z].alt_img;
get_logo.dataset.votos= grilla[z].votos;
get_logo.dataset.vistos= grilla[z].vistos;
get_logo.dataset.alta= grilla[z].fecha;
}else{



get_logo_est_clon.getElementsByTagName("img")[0].src = grilla[z].link;
get_logo_est_clon.getElementsByTagName("img")[0].alt = "texto imagen";
get_logo_est_clon.getElementsByTagName("h2")[0].innerHTML = grilla[z].titulo;
get_logo_est_clon.getElementsByTagName("h3")[0].innerHTML = grilla[z].nombre;
get_logo_est_clon.getElementsByTagName("img")[1].src = grilla[z].avatar;
get_logo_est_clon.getElementsByTagName("img")[1].alt = grilla[z].alt_img;
get_logo_est_clon.dataset.votos= grilla[z].votos;
get_logo_est_clon.dataset.vistos= grilla[z].vistos;
get_logo_est_clon.dataset.alta= grilla[z].fecha;
get_logo_gral.appendChild(get_logo_est_clon);
}
...
function filtros_js(contenedor_gral_logos, filtro_click, borrar_cont, filtrado, grilla_array){
    ...
    for (var z = 0; z < grilla_array.length; z++){
        // var get_logo_est_clon = get_logo.cloneNode(true);
        get_logo_est_clon = backup_estructura.cloneNode(true);
        get_logo_est_clon.getElementsByTagName("img")[0].src = grilla_array[z].link;
        get_logo_est_clon.getElementsByTagName("img")[0].alt = "texto imagen";
        get_logo_est_clon.getElementsByTagName("h2")[0].innerHTML = grilla_array[z].titulo;
        get_logo_est_clon.getElementsByTagName("h3")[0].innerHTML = grilla_array[z].nombre;
        get_logo_est_clon.getElementsByTagName("img")[1].src = grilla_array[z].avatar;
        get_logo_est_clon.getElementsByTagName("img")[1].alt = grilla_array[z].alt_img;
        get_logo_est_clon.dataset.votos= grilla_array[z].votos;
        get_logo_est_clon.dataset.vistos= grilla_array[z].vistos;
        get_logo_est_clon.dataset.alta= grilla_array[z].fecha;
        contenedor_gral_logos.appendChild(get_logo_est_clon);
    }
		...
		if (in_sp[this.numerito].classList.contains("editar")) {
			...
			if (in_sp[this.numerito].getElementsByTagName("input")[0].type == "file") {
				...
				if (in_sp[this.numerito].getElementsByTagName("input")[0].id == "file-1") {
					get_span.innerHTML = "Cambiar imagen";
				}
